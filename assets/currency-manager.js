...
class CurrencyManager {
	...
	async handleCurrencyChange() {
		...
		if (newCurrency !== this.currentCurrency) {
			console.log(
				`🔄 Currency changed from ${this.currentCurrency} to ${newCurrency}`
			);
	...
	updateExistingPriceDisplays() {
		...
		document
			.querySelectorAll("[data-currency-text][data-amount]")
			.forEach((element) => {
	...
	async fetchGiftBoxPricing() {
		try {
			const giftBoxHandle = "premium-gift-box-and-tissue-wrap";
      ...
      `;
			...
			if (product && product.variants && product.variants.edges.length > 0) {
				...
				console.log(
					`🎁 Gift box price fetched via GraphQL: ${variant.price.currencyCode} ${variant.price.amount} (${priceInCents} cents)`
				);
