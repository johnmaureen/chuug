...
class ContentVideoSection {
	...
	setupVideoInteraction() {
		...
		const videoEmbeds = this.section?.querySelectorAll(
			".content-video-section__video-embed iframe"
		);
		...
		const videoElement = this.section?.querySelector(
			".content-video-section__video-element"
		);
		if (videoElement) {
			videoElement.addEventListener("play", () => this.trackVideoPlay("html5"));
			videoElement.addEventListener("pause", () =>
				this.trackVideoPause("html5")
			);
		...
		const videoImage = this.section?.querySelector(
			".content-video-section__video-image"
		);
		if (videoImage) {
	...
	setupAccessibility() {
		...
		const content = this.section?.querySelector(
			".content-video-section__content"
		);
